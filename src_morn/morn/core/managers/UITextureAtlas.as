 ...
 *  <code>uiframe</code> property of the Texture class. Trim the texture by removing the
 ...
 *  form), while the "uiframe" properties must describe the texture in its upright form.</p>
 *
 */
public class UITextureAtlas {
    ...
    public function UITextureAtlas(texture:BitmapData, atlasXml:XML = null) {
    ...
    public function dispose():void {
        ...
        for (var key:String in _subTextures){
            if (_subTextures[key] && _subTextures[key] is BitmapData) {
                (_subTextures[key] as BitmapData).dispose();
            }
            delete _subTextures[key];
        }
    }
    ...
    protected function parseAtlasXml(atlasXml:XML):void {
        var scale:Number = 1;//_atlasTexture.scale;
    ...
    public function getTextures(prefix:String = "", out:Vector.<BitmapData> = null):Vector.<BitmapData> {
     ...
     *  points) with an optional uiframe. */
    public function addRegion(name:String, region:Rectangle, frame:Rectangle = null,
                              rotated:Boolean = false):void {
        _subTextures[name] = createSubTextureBitmapData(_atlasTexture, region, false, frame, rotated);
        _subTextureNames = null;
    }

    private function createSubTextureBitmapData(parent:BitmapData, region:Rectangle = null,
                                                ownsParent:Boolean = false, frame:Rectangle = null,
                                                rotated:Boolean = false, scaleModifier:Number = 1):BitmapData {
        if (region == null) {
            region = new Rectangle();
            region.setTo(0, 0, parent.width, parent.height);
        }
        if (frame == null) {
            frame = new Rectangle();
            var _width:Number = (rotated ? region.height : region.width) / scaleModifier;
            var _height:Number = (rotated ? region.width : region.height) / scaleModifier;
            frame.setTo(0, 0, _width, _height);
        }

        var w:Number = (rotated ? frame.height : frame.width) / scaleModifier;
        var h:Number = (rotated ? frame.width : frame.height) / scaleModifier;

        var bmd:BitmapData = new BitmapData(w / scaleModifier, h / scaleModifier, true, 0);
        bmd.copyPixels(parent, region, new Point(-frame.x, -frame.y));
        var dstBmd:BitmapData;

        if (rotated) {
            dstBmd = new BitmapData(frame.width, frame.height, true, 0);
            var matrix:Matrix = new Matrix();
            matrix.rotate(Math.PI / 2);
            matrix.scale(-1, -1);
            matrix.translate(0, frame.height);
            dstBmd.draw(bmd, matrix);
        } else {
            dstBmd = bmd;
        }
    ...
    public function removeRegion(name:String):void {
        var subTexture:BitmapData = _subTextures[name];
