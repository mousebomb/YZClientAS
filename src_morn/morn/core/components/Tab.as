...
package morn.core.components {
	...
	public class Tab extends Box implements IItem {
		...
		/**增加Radio*/
		public function addItem(button:UIButton):void {
		...
		override public function set skin(value:String):void {
			if (super.skin != value) {
				...
				for (var i:int = 0; i < int.MAX_VALUE; i++) {
					var item:UIButton = getChildByName("item" + i) as UIButton;
					if (item == null) {
						break;
					}
					item.useTextureSkin = useTextureSkin;
					item.skin = value;
				}
			...
			if (_labels != value) {
				...
				if (Boolean(_labels)) {
					...
					for (var i:int = 0, n:int = a.length; i < n; i++) {
						var btn:UIButton = createButton(skin, a[i], useTextureSkin);
						btn.name = "item" + i;
						addChild(btn);
					}
				}
		...
		protected function createButton(skin:String, label:String, useTextureSkin:Boolean):UIButton {
			return new UIButton(skin, label, useTextureSkin);
		}
		...
		protected function changeLabels():void {
			...
			for (var i:int = 0, n:int = _items.length; i < n; i++) {
				var btn:UIButton = _items[i] as UIButton;
