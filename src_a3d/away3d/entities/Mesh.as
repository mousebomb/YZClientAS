...
{
	import away3DExtend.SkinnedSubGeometryExtend;
	...
	public class Mesh extends Entity implements IMaterialOwner, IAsset
	{
		...
		private var _castsShadows:Boolean = false;//true;
		...
		{
			if(geometry) geometry.disposeForStage3D();
			material = null;
			geometry = null;
...
/***********************************************  Extend  **************************************************/
		 ...
		 * @return
		 */
		public function getSubMeshForName(name:String):SubMesh
		{
			for each(var subMesh:SubMesh in _subMeshes)
			{
				if(SkinnedSubGeometryExtend(subMesh.subGeometry).noteData.meshes == name)
					return subMesh;
	}
			return null;
}
