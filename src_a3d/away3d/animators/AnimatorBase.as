package away3d.animators
{
	...
	public class AnimatorBase extends NamedAssetBase implements IAsset
	{
		...
		protected var _playTime:Number = 0;	//当前动作播放时间
		 ...
		 */
		public var updatePosition:Boolean = false;

		public function getAnimationState(node:AnimationNodeBase):AnimationStateBase
		{
			if (node is CrossfadeTransitionNode)
			{
				return node.getState(this, node);
			}
		...
		{
			if(!_activeState || !_animationSet)
			{
				if(this is SkeletonAnimator)
					return;
			}
			...
			{
				StageFrame.addAnimFun(onEnterFrame);
				//				MyAway3DFrameWorker.instance.addFun(this, onEnterFrame);
		...
		public function stop():void
		{
			...
			//			if (_broadcaster.hasEventListener(Event.ENTER_FRAME))
			{
				StageFrame.removeAnimFun(onEnterFrame);
				//				MyAway3DFrameWorker.instance.removeFun(this);
		...
		/*
		 private function get forceStop():Boolean
		 {
		 var tmpLen:int = _owners.length;
		 for (var i:int = 0; i < tmpLen; ++i)
		 {
		 if (_owners[i].isInFrustumVisible)
		 {
		 return false;
		 }
		 }
