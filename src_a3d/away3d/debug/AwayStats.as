package away3d.debug
{
	...
	public class AwayStats extends Sprite
	{
		 ...
		 * after construction using the registerView() method. Omit the game.view
		 * constructor parameter to disable this feature altogether.</p>
		 *
		 * @param game.view A reference to your Away3D game.view. This is required if you
		...
		/**
		 * Add a game.view to the list of those that are taken into account when
		 ...
		 * your game.view, or when using several views, or when views are created and
		...
		/**
		 * Remove a game.view from the list of those that are taken into account when
		 * calculating on-screen and total poly counts. If the supplied game.view is
		...
		private function _redrawWindow():void
		{
			...
			// Hide/show diagram for minimized/maximized game.view respectively
		...
		private function _redrawStats():void
		{
			...
			// Only redraw polycount if there is a  game.view available
			...
			if (_views.length > 0) {
				...
				_poly_tf.text = 'n/a (no game.view)';
			...
			if (!_showing_driv_info) {
				if (_views && _views.length && _views[0].renderer.stage3DProxy && _views[0].renderer.stage3DProxy.context3D) {
					...
					_swhw_tf.text = 'n/a (no game.view)';
			}
