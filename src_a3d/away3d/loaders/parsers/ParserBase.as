package away3d.loaders.parsers
{
	...
	/**
	 * <code>ParserBase</code> provides an abstract base class for objects that convert blocks of data to data
	 * structures supported by Away3D.
	 *
	 ...
	 * create the object that will contain the parsed data. This allows <code>ResourceManager</code> to return an
	 * object
	...
	public class ParserBase extends EventDispatcher
	{
		...
		private static var _parsingVec:Vector.<ParserBase> = new Vector.<ParserBase>();

		/** 1帧内最多在多少毫秒内允许继续解析 */
		public static const MAX_EXECPARSE_TIME :int = 4;
		/** 单个任务1帧内最大允许继续解析的耗时 */
		public static const MAX_EXECPARSE_ONE_TIME :int = 3;
		/**每帧调用一次 解析方法； 内部控制最大执行时间 */
		public static function execParse():void
		{
			var len:int = _parsingVec.length;
			if (len > 0)
			{
				var tmpParser:ParserBase;
				var tmpTime:uint = getTimer();
				for (_tmpIdx = 0; _tmpIdx < _parsingVec.length; ++_tmpIdx)
				{
					// 一帧内总的控制时间
					if (getTimer() - tmpTime < MAX_EXECPARSE_TIME)
					{
						if (_tmpIdx >= 0)
						{
							tmpParser = _parsingVec[_tmpIdx];
							tmpParser.execParsing();
						}
					}
					else
					{
//						trace("帧",StageFrame.renderIdx,"ParserBase/execParse 耗时过大 开始",tmpTime-StageFrame.curTime,"耗时", getTimer() - StageFrame.curTime , "剩余",_parsingVec.length);
						break;
					}
				}
			}
		...
		private static var _tmpIdx:int = 0;
		private static function addParser(parse:ParserBase):void
		{
			var idx:int = _parsingVec.indexOf(parse);
			if (idx == -1)
			{
				_parsingVec.push(parse);
			}
		}

		private static function removeParser(parse:ParserBase):void
		{
			var idx:int = _parsingVec.indexOf(parse);
			if (idx != -1)
			{
				var isLast:Boolean = idx == _parsingVec.length - 1;
				if (isLast == false)
				{
					_parsingVec[idx] = _parsingVec[_parsingVec.length - 1];
//					_parsingVec[idx].execParsing();
					_tmpIdx--;
				}
				_parsingVec.pop();
			}
		 ...
		 * @param format The data format of the file data to be parsed. Can be either
		 *     <code>ParserDataFormat.BINARY</code> or <code>ParserDataFormat.PLAIN_TEXT</code>, and should be provided
		 *     by the concrete subtype.
		 *
		 ...
		 * Options:
		 * 0 (Default / undefined) - All Parsers will create SinglePassMaterials, but the AWD2.1parser will create
		 * Materials as they are defined in the file
		...
		/**
		 * The data format of the file data to be parsed. Can be either <code>ParserDataFormat.BINARY</code> or
		 * <code>ParserDataFormat.PLAIN_TEXT</code>.
		 */
		 ...
		 */
		public function parseAsync(data:*):void
		{
			_data = data;
			startParsing();
		}
		...
		/**
		 * Resolve a dependency when it's loaded. For example, a dependency containing an ImageResource would be
		 * assigned to a Mesh instance as a BitmapMaterial, a scene graph object would be added to its intended parent.
		 * The dependency should be a member of the dependencies property.
		 *
		...
		/**
		 * Finalize a constructed asset. This function is executed for every asset that has been successfully
		 * constructed. It will dispatch a <code>AssetEvent.ASSET_COMPLETE</code> and another AssetEvent, that depents
		...
		protected function addDependency(id:String, req:URLRequest, retrieveAsRawData:Boolean = false,
		 ...
		 * @return True if there is still time left, false if the maximum allotted time was exceeded and parsing should
		 *     be interrupted.
		 */
		protected function hasTime():Boolean
		{
			// 一帧内单个解析对象分配时间片
			return ((getTimer() - _lastFrameTime) < MAX_EXECPARSE_ONE_TIME);
		}

		/**
		 * Called when the parsing pause interval has passed and parsing can proceed.
		 */
		private function execParsing():void
		{
