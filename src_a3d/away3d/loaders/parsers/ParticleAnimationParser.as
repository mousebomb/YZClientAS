	...
	public class ParticleAnimationParser extends CompositeParserBase
	{
		...
		override protected function proceedParsing():Boolean
		{
			if (_isFirstParsing)
			{
				...
				/*
				if (_data.name && id == "TextureMaterialSubParser")
				{
					var tmpArr:Array = _data.name.split("_");
					var tmpVal:String = tmpArr[tmpArr.length - 1];
					_particleMaterialParser.depthCompareMode = tmpVal == "always" ? Context3DCompareMode.ALWAYS : Context3DCompareMode.LESS_EQUAL;
				}
				*/
				...
				for each (var nodedata:Object in nodeDatas)
				{
					...
					if (!parserCls)
					{
						dieWithError("Unknown node parser-->" + id);
					}
				...
				if (globalValuesDatas)
				{
					...
					for each (var valuedata:Object in globalValuesDatas)
					{
						...
						idStr = valuedata.id;
						if (idStr.substr(0, 3) != "Lua")
						{
							parserCls = MatchingTool.getMatchedClass(idStr, AllSubParsers.ALL_GLOBAL_VALUES);
							if (!parserCls)
							{
								dieWithError("Unknown node parser-->" + id);
							}
		...
		private function generateAnimation():void
		{
			...
			//mesh:
			_particleMesh = new MeshExtend(_particlegeometryParser.particleGeometry, _particleMaterialParser.material);
