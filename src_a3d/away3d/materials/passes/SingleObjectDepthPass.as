package away3d.materials.passes
{
	...
	public class SingleObjectDepthPass extends MaterialPassBase
	{
		...
		arcane override function render(renderable:IRenderable, stage3DProxy:Stage3DProxy, camera:Camera3D, viewProjection:Matrix3D):void
		{
			...
			var target:Texture ;
			if( _textures[contextIndex][renderable] ){
				target = _textures[contextIndex][renderable];
			}else{
				++Debug.texturesNum;
				target = _textures[contextIndex][renderable] = context.createTexture(_textureSize, _textureSize, Context3DTextureFormat.BGRA, true);
			}
