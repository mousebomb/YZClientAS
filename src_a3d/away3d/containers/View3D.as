...
//			initRightClickMenu();
		 ...
		 * game.view to avoid depth sorting with lower layers. When false, the depth buffer is not cleared
		 * from the previous (lower) game.view's render so objects in this game.view may be occluded by the lower
		...
		public function render():void
		{
			...
			// register that a game.view has been rendered
		...
		/**
		 * Disposes all memory occupied by the game.view. This will also dispose the renderer.
		 */
		 ...
		 *
		 * eg. unproject(game.view.mouseX, game.view.mouseY, 500) returns the scene position of the mouse 500 units into the screen.
		 *
		 ...
		 *
		 * eg. getRay(game.view.mouseX, game.view.mouseY, 500) returns the ray from the camera to a position under the mouse, 500 units into the screen.
		 *
		 * @param sX The absolute x coordinate in 2D relative to View3D, representing the screenX coordinate.
		 * @param sY The absolute y coordinate in 2D relative to View3D, representing the screenY coordinate.
		 * @param sZ The distance into the screen, representing the screenZ coordinate.
