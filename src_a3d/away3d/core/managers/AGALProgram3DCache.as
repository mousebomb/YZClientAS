package away3d.core.managers
{
	...
	public class AGALProgram3DCache
	{
		...
		/** 统计透过我上传shader的次数 */
		public static var statUploadProgramTimes:uint = 0;
		/** 当不存在的shader被首次缓存下来的回调 */
		public static var cachedCallback:Function;
		public function setProgram3D(pass:MaterialPassBase, vertexCode:String, fragmentCode:String):void
		{
			...
			if (_program3Ds[key] == null) {
				...
				*/
				...
				if(PrecompiledShaders.contains(key))
				{
					 vertexByteCode=PrecompiledShaders.getVertexShader(key);
					 fragmentByteCode=PrecompiledShaders.getFragmentShader(key);
				}else{
				...
				if(cachedCallback !=null)
					cachedCallback(key ,vertexByteCode,fragmentByteCode);
				++statUploadProgramTimes;
				//				trace("-------------------------------------------->Program3D.upload:", getTimer() - tmpTime, vertexByteCode.length, fragmentByteCode.length);
