...
package feathers.controls
{
	...
	/**
	 * Dispatched when the scroller scrolls in either direction or when the game.view
	...
	/**
	 * Allows horizontal and vertical scrolling of a <em>game.view port</em>. Not
	...
	public class Scroller extends FeathersControl implements IFocusDisplayObject
	{
		...
		/**
		 * Determines if the dimensions of the game.view port are used when measuring
		 * the scroller. If disabled, only children other than the game.view port
		 * (such as the background skin) are used for measurement.
		 *
		 * <p>In the following example, the game.view port measurement is disabled:</p>
		 *
		 * <listing version="3.0">
		 ...
		 *
		 * <p>If the scroller's game.view port supports infinite scrolling, this
		...
		protected function refreshViewPortBoundsForMeasurement():void
		{
			...
			//we account for the explicit minimum dimensions of the game.view port
			...
			//event, which forces another layout pass for the game.view port and
			...
			//game.view port fills the entire bounds.
			...
			this._viewPort.validate();
			//we don't want to listen for a resize event from the game.view port
			//while it is validating this time. during the next validation is
			//where it matters if the game.view port resizes.
		...
		protected function refreshViewPortBoundsForLayout():void
		{
			...
			//we'll only set the game.view port's visibleWidth and visibleHeight if
			//our dimensions are explicit. this allows the game.view port to know
			...
			//game.view port is validating because it means we'll need to try another
