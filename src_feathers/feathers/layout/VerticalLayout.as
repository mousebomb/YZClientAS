...
package feathers.layout
{
	...
	public class VerticalLayout extends EventDispatcher implements IVariableVirtualLayout, ITrimmedVirtualLayout, IGroupedLayout
	{
		 ...
		 * header will stick to the top of the game.view port until the current group
		 * completely scrolls out of the game.view port.
		 *
		 * @default false
		 */
		...
		/**
		 * Distributes the height of the game.view port equally to each item. If the
		 * game.view port height needs to be measured, the largest item's height will
		...
		/**
		 * Requests that the layout set the game.view port dimensions to display a
		 * specific number of rows (plus gaps and padding), if possible. If the
		 * explicit height of the game.view port is set, then this value will be
		 * ignored. If the game.view port's minimum and/or maximum height are set,
		...
		public function layout(items:Vector.<DisplayObject>, viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult
		{
			...
			for(var i:int = 0; i < itemCount; i++)
			{
				...
				if(nextHeaderIndex === iNormalized)
				{
					...
					//the game.view port. the previous one should be sticky.
				...
				else
				{
					...
					if(this._useVirtualLayout)
					{
						if(this._hasVariableItemDimensions)
						{
							if(itemHeight != cachedHeight)
							{
								...
								//changed, and it the game.view port may need to be
								//re-measured.
					...
					if(itemWidth > maxItemWidth)
					{
						...
						//case where the width of the game.view port needs to be
			...
			{
				//position the "sticky" header at the top of the game.view port.
			...
			//the available height of the game.view port.
		...
		public function getVisibleIndicesAtScrollPosition(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int> = null):Vector.<int>
		{
			...
			//we add one extra here because the first item renderer in game.view may
